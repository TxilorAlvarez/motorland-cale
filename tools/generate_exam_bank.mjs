...
const rows = questions.map(question => {
    ...
    const reference = "Material pedagógico basado en Manual de referencia para la conducción de vehículos ANSV";
    return `(${escape(question.type)},${escape(question.module)},ARRAY[${question.categories.map(escape).join(",")}],${escape("medium")},${escape(clean(question.prompt))},${escape(clean(question.options.A))},${escape(clean(question.options.B))},${escape(clean(question.options.C))},${escape("Ninguna de las anteriores.")},${escape(question.answer)},${escape("Respuesta de preparación incluida en el material pedagógico de Motorland.")},${escape(source)},NULL,${escape(reference)})`;
});
